    ...
    public class Cal
    {
        ...
        internal abstract class Interpolation : IInterpolate
        {
            public Interpolation(double[] _x, double[] _y)
            {
                ...
                if (xLength == _y.Length && xLength > 1 && _x.Count() == xLength) // _x.Distinct().Count()
                {
                    x = _x;
                    y = _y;
                }
            ...
            public Interpolation(double[] _x, double[] _y, bool checkSorted = true)
            {
                ...
                if (checkSorted)
                {
                    if (xLength == _y.Length && xLength > 1 && _x.Count() == xLength && Enumerable.SequenceEqual(_x.SortedList(), _x.ToList()))// _x.Distinct().Count()
                    {
                        x = _x;
                        y = _y;
                    }
        ...
        #endregion
        #region LINEAR
        internal  class LinearInterpolation : Interpolation
        {
