...
namespace _2_2_700_1300nm_alpha_beta
{
    class Cal
    {
        ...
        {
            StreamWriter streamWriter = new StreamWriter(new FileStream("700~1300nm_alpha_beta.txt", FileMode.Create));
            streamWriter.WriteLine("wave(nm)\t\t AOI\t alpha\t\t\t beta");

            double sio2_1000_nm = 0.0;
            double sio2_1000_AOI = 0.0;
            double sio2_1000_alpha = 0.0;
            double sio2_1000_beta = 0.0;
            double sio2_nm = 0.0;
            double sio2_n = 0.0;
            double sio2_k = 0.0;
            double si_nm = 0.0;
            ...
            {
                sio2_1000_nm = Convert.ToSingle(SiO2_1000_records[i].nm);
                sio2_1000_AOI = Convert.ToSingle(SiO2_1000_records[i].AOI);
                sio2_1000_alpha = Convert.ToSingle(SiO2_1000_records[i].alpha);
                sio2_1000_beta = Convert.ToSingle(SiO2_1000_records[i].beta);
                ...
                if (si_nm > 350 && si_nm < 980)
                {
                    ...
                    // 두께 = thick
                    int thick = 0;
                    for (thick = 700; thick <= 1300; thick += 5)
                    {
                        // Beta -> 위상 두께                    
                        Complex Beta_thick = (2 * Math.PI * thick * N1 * costheta1) / sio2_nm;
                        ...
                        streamWriter.WriteLine("{0}\t {1}\t {2}\t {3}", si_nm, thick, alpha, beta);
                    }
                }
            }
            streamWriter.Close();
            WriteLine("두께별 alpha_beta.dat 생성 완료");
        }
    }
}
